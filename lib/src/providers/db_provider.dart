...
class DBProvider {
  ...
  initDB() async{
    Directory documentosDirectorio = await getApplicationDocumentsDirectory();
    final path = join( documentosDirectorio.path,'ScansDB.db');
  ...
  Future<List<ScanModel>> obtenerTodosScan() async {
    final db = await database;
  ...
  Future<List<ScanModel>> obtenerTodosScanTipo( String tipo) async {
    final db = await database;
